...
def query_cluster(cluster_path=None, query = None):
    ...
    for closest_cluster_index in chunk_embeddings:   
        ...
        for closest_cluster_index, cluster_embeddings in updated_cluster_embeddings.items():
            for embedding in cluster_embeddings:
                # Calculate Euclidean distance
                embedding_array = np.array(embedding)
                distance = np.linalg.norm(embedding_array - query_embedding)
                
                # Assuming `embedding` has associated doc_id and chunk_index
                doc_id = embedding['doc_id']
                chunk_index = embedding['chunk_index']
                
                # Append distance and identifiers to the list
                distances.append((distance, doc_id, chunk_index))

        # Sort by distance
        distances.sort(key=lambda x: x[0])
        # Output sorted distances
        for distance, doc_id, chunk_index in distances:
            print(f"doc_id: {doc_id}, chunk_index: {chunk_index}, distance: {distance}")
...
def save_last_document_id():
    with open(doc_id_file_path, 'wb') as f:
        pickle.dump(doc_id_counter - 1, f)
